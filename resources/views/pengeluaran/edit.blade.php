                ...
                <form action="{{ route('pengeluaran.update', $pengeluaran->id_pengeluaran) }}" method="POST"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-1">
                            ...
                            <select name="id_proyek" id="id_proyek" class="w-full ...">
                                <option value="">-- Pilih Proyek --</option>
                                @foreach ($proyek as $p)
                                    <option value="{{ $p->id_proyek }}" data-nama="{{ $p->nama_proyek }}"
                                        {{ old('id_proyek', $pengeluaran->id_proyek) == $p->id_proyek ? 'selected' : '' }}>
                                        {{ $p->nama_proyek }}
                                    </option>
                                @endforeach
                            </select>
                            <input type="hidden" name="nama_proyek" id="nama_proyek"
                                value="{{ old('nama_proyek', $pengeluaran->nama_proyek) }}">
                        </div>
                    ...
                    <div class="space-y-1">
                        ...
                        <textarea name="keterangan" id="keterangan" class="w-full ..." rows="3">{{ old('keterangan', $pengeluaran->keterangan) }}</textarea>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function resetVendors() {
                ...
                vendorEl.classList.add('bg-gray-50');
                vendorEl.classList.remove('bg-white');
            }
            ...
            function resetRek() {
                ...
                rekeningEl.classList.add('bg-gray-50');
                rekeningEl.classList.remove('bg-white');
            ...
            jenisEl.addEventListener('change', async function() {
                ...
                try {
                    ...
                    if (Array.isArray(data) && data.length > 0) {
                        ...
                        vendorEl.classList.remove('bg-gray-50');
                        vendorEl.classList.add('bg-white');
                    }
                } catch (err) {
            ...
            vendorEl.addEventListener('change', function() {
                ...
                try {
                    ...
                    if (Array.isArray(arr) && arr.length > 0) {
                        ...
                        rekeningEl.classList.remove('bg-gray-50');
                        rekeningEl.classList.add('bg-white');
