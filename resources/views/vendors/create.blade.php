...
<div class="container mx-auto p-4 sm:p-6 lg:p-8">
    ...
    {{-- Pesan Error --}}
    @if ($errors->any())
        <div class="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg mb-6 dark:bg-red-900/20 dark:border-red-800 dark:text-red-300">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-6">
        <form action="{{ route('vendors.store') }}" method="POST" class="space-y-6">
            ...
            <div class="border-b border-gray-200 dark:border-gray-700 pb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        <label for="nama_vendor" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    ...
                    {{-- Jenis Vendor --}}
                    <div>
                        <label for="jenis_vendor" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Jenis Vendor
                        </label>
                        <select name="jenis_vendor" id="jenis_vendor"
                            ...
                            <option value="Tukang" {{ old('jenis_vendor') == 'Tukang' ? 'selected' : '' }}>Tukang</option>
                            <option value="Material" {{ old('jenis_vendor') == 'Material' ? 'selected' : '' }}>Material</option>
                            <option value="Jasa dan Material" {{ old('jenis_vendor') == 'Jasa dan Material' ? 'selected' : '' }}>Jasa dan Material</option>
                        </select>
                    </div>
            ...
            <div class="border-b border-gray-200 dark:border-gray-700 pb-6">
                ...
                <div id="rekening-wrapper" class="space-y-4">
                    ...
                    @foreach ($rekening_list as $key => $rekening)
                        <div class="rekening-group p-4 border border-gray-200 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                            <div class="flex items-center justify-between mb-4">
                                <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300">Rekening {{ $key + 1 }}</h4>
                                @if ($key > 0)
                                    <button type="button" class="remove-rekening text-gray-400 hover:text-red-500 dark:hover:text-red-400 transition-colors duration-200 p-1" title="Hapus rekening">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="rekening[{{ $key }}][nama_bank]" list="bank_options" 
                                        class="w-full h-10 px-3 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200" 
                                        value="{{ $rekening['nama_bank'] ?? '' }}">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">No Rekening</label>
            ...
            <div class="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                <a href="{{ route('vendors.index') }}"
                    class="w-full sm:w-auto bg-gray-500 hover:bg-gray-600 text-white font-medium py-2 px-6 rounded-lg shadow-sm text-sm text-center transition duration-200">
                    Batal
                </a>
                <button type="submit"
                    class="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-6 rounded-lg shadow-sm text-sm transition duration-200">
                    Simpan
                </button>
            </div>
        </form>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const createNewRekeningGroup = (index) => {
        ...
        newRekeningGroup.className = 'rekening-group p-4 border border-gray-200 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700/50';
        newRekeningGroup.innerHTML = `
            <div class="flex items-center justify-between mb-4">
                <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300">Rekening ${index + 1}</h4>
                <button type="button" class="remove-rekening text-gray-400 hover:text-red-500 dark:hover:text-red-400 transition-colors duration-200 p-1" title="Hapus rekening">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                    </svg>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Atas Nama</label>
                    <input type="text" name="rekening[${index}][atas_nama]" class="w-full h-10 px-3 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Nama Bank</label>
                    <input type="text" name="rekening[${index}][nama_bank]" list="bank_options" class="w-full h-10 px-3 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">No Rekening</label>
                    <input type="text" name="rekening[${index}][no_rekening]" class="w-full h-10 px-3 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200">
                </div>
            </div>
        `;
