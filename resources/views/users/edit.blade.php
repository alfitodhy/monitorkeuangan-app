...
<div class="container mx-auto px-4 py-8 md:px-8 lg:px-12">
    <div class="max-w-5xl mx-auto bg-white dark:bg-gray-900 rounded-2xl shadow-xl p-6 md:p-10">
        <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-8 text-left">Edit Pengguna: {{ $user->nama_lengkap }}</h2>
        
        <form action="{{ route('users.update', $user->id_user) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="relative">
                            <label for="password" class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">
                                Password
                                <span class="text-xs text-gray-500 dark:text-gray-400 font-normal ml-1">(Kosongkan jika tidak diubah)</span>
                            </label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="role" id="role" 
                                ...
                                <option value="kepala operational" {{ old('role', $user->role) == 'kepala operational' ? 'selected' : '' }}>Kepala Operational</option>
                                <option value="bod" {{ old('role', $user->role) == 'bod' ? 'selected' : '' }}>BOD</option>
                ...
                <div class="lg:col-span-1 flex flex-col items-center justify-center space-y-4">
                    ...
                    <div id="image-container" class="relative h-40 w-40 rounded-full border-4 border-gray-200 dark:border-gray-700 shadow-lg bg-gray-100 dark:bg-gray-800 flex items-center justify-center overflow-hidden">
                        <img id="image-preview" 
                             src="{{ $user->foto_profil ? asset('storage/' . $user->foto_profil) : 'https://via.placeholder.com/150/E5E7EB/6B7280?text=' }}" 
                             alt="Foto Profil" 
                             class="absolute inset-0 h-full w-full object-cover {{ $user->foto_profil ? '' : 'hidden' }}">
                        <svg id="default-user-icon" xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="w-16 h-16 text-gray-400 dark:text-gray-600 {{ $user->foto_profil ? 'hidden' : '' }}" viewBox="0 0 24 24">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Cek apakah ada foto profil yang sudah ada saat halaman dimuat
        const initialImageSrc = '{{ $user->foto_profil ? asset('storage/' . $user->foto_profil) : '' }}';
        if (initialImageSrc) {
            uploadButtonText.textContent = 'Ganti Foto';
        } else {
            uploadButtonText.textContent = 'Pilih Foto';
        }
